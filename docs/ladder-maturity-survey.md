...
| 문서 | 축 |
| --- | --- |
| **`commercial-rl-survey.md`** | **상용화.** 실제로 팔리고 매출·운영에 들어간 것만 |
...
> **⚠ 이 문서의 등급은 "연구·프레임워크 성숙도"입니다 — 상용화 수준이 아닙니다.**
> 둘은 크게 갈립니다. 예: **조작은 여기서 L3가 `상용` 등급이지만, 실제 상용 창고 픽킹은 대부분 RL이 아니라
> 지도학습·파운데이션 모델입니다** ([상용 서베이 6절](commercial-rl-survey.md)).
> **"업계가 어디까지 왔나"를 물으면 상용 서베이를, "기법이 얼마나 정착했나"를 물으면 이 문서를 보십시오.**
...
**등급은 행이 아니라 칸 단위입니다.** 같은 칸이 도메인마다 다른 등급이고, **행을 통째로 읽으면 오독합니다.**
예: L3는 조작에서 **상용**이지만 로코모션에서 **확산 중**입니다 — 성숙도가 낮아서가 아니라
**로코모션이 시연을 안 쓰기 때문**입니다(맨땅 RL로 충분). 낮은 등급이 "덜 발전함"을 뜻하지 않는 칸이 있습니다.

**`상용`은 두 조건을 모두 요구합니다** — ①여러 팀의 일상 사용 ②라이브러리 기본값 제공.
**하나만 충족하면 `정착`으로 내렸습니다.**

| 등급 | 뜻 |
| --- | --- |
| **상용** | 여러 팀이 재현 절차로 일상 사용 **그리고** 라이브러리 기본값에 들어가 있음 (둘 다) |
...
| **L2a** RL → 저수준 컨트롤러 setpoint | **상용** | **상용** | **상용** | **상용** | **상용** |
| **L2b** 명시적 잔차 (MPC·임피던스) | **정착** ↑ | 확산 중 | **정착** | 확산 중 | 정착 |
...
- **L2a가 전 도메인 상용입니다** — 유일하게 한 줄이 전부 상용인 칸입니다. **순수 엔드투엔드 토크 제어는 사실 드뭅니다.** 4.1절
- **L0의 분포가 극단적입니다** — 로코모션·드론에서 상용, 조작·손재주에서 제한적. **같은 기법이 도메인에 따라 정반대 등급입니다**
- **L4가 학습 기법 중 가장 고르게 높습니다** — 어느 열에도 "연구"가 없습니다. 이게 무게 중심입니다
...
## 4. L2 — 두 갈래입니다. 한쪽은 이미 전부가 쓰고 있습니다

> **⚠ 이 절은 등급 오류를 고친 것입니다.** 초판에서 L2 행에 **상용 칸이 하나도 없었는데 그것이 오류였습니다.**
> L2를 "모델 기반 컨트롤러에 명시적 잔차를 더하는 것"으로만 좁게 읽었고, **그렇게 읽었다고 밝히지 않았습니다.**
> 넓은 쪽(L2a)은 **이미 전 도메인 라이브러리 기본값**입니다.

### 4.1 L2a — RL이 저수준 컨트롤러에 setpoint를 낸다: 상용 `[문헌]`

**Isaac Lab의 기본 제어 법칙이 이미 L2입니다.**

```
tau = Kp*(q_default + Ka*a - q) - Kd*dq        # Kp/Kd: 강성·감쇠, Ka: 정책 행동 스케일
```

`[문헌]` 정책은 **토크를 내지 않습니다.** 기본 관절 자세(`q_default`) 주변의 **관절 위치 목표를 파라미터화**하는
정규화된 행동을 내고, **그 목표를 관절별 PD 컨트롤러가 더 높은 주기로 추종**합니다.
Isaac Lab 문서가 이걸 **계층 제어(hierarchical control)** 로 서술합니다.

**즉 "맨땅 PPO 로코모션"조차 고전 컨트롤러 위에서 돕니다.** 사전지식이 두 군데 들어가 있습니다.

| 어디에 | 무엇이 |
| --- | --- |
| `q_default` | **기본 관절 자세** — 정책이 탐색할 출발 자세를 사람이 정해 준 것 |
| `Kp` · `Kd` · `Ka` | **저수준 추종 동역학** — 정책이 배우지 않는 부분 |

**도메인 전반에서 같습니다** `[문헌]`:

| 도메인 | 정책 출력 | 저수준이 하는 일 |
| --- | --- | --- |
| 로코모션 | 관절 위치 목표·델타 | 관절별 PD 추종 |
| 드론 | **추력 + 바디레이트** | 레이트 컨트롤러 추종 |
| 조작 | 관절 위치 목표, 또는 **엔드이펙터 자세 → IK** | PD·IK·오퍼레이셔널 스페이스 |
| 손재주 | 손가락 관절 위치 목표 | PD 추종 |
| 산업 배치 | 자세·힘 목표 | **임피던스 제어** |

**그래서 이 줄이 전 도메인 상용입니다. 순수 엔드투엔드 토크 제어는 오히려 특수한 선택입니다.**

`[미확인]` **우리 태스크가 어느 쪽인지 확인하십시오** — cartpole Direct는 effort(토크) 기반일 수 있습니다.
`Isaac-Lift-Cloth-Franka-v0`의 action term이 무엇인지 보는 것이 cable 태스크 설계의 첫 결정입니다.

### 4.2 L2b — 태스크 수준 모델 기반 컨트롤러에 명시적 잔차: 정착

**앞 문서는 이쪽을 2018년 Siemens residual RL로 소개했습니다. 지금은 그 그림이 낡았습니다.**
...
**이게 왜 중요한가.** L2b는 "RL이 약할 때 쓰는 우회로"가 아닙니다.
**모델 기반 제어를 사전지식으로 넣으면 보상 설계 부담이 줄어든다**는 것이 이 칸의 값이고,
앞 문서 6.5절(보상 거버넌스)과 직접 연결됩니다 — **보상 항이 적으면 해킹 표면도 작습니다.**

### 4.3 L2a/L2b 구분이 실무에서 갖는 뜻 `[자작]`

**"L2를 도입할까"는 잘못된 질문입니다.** L2a는 이미 쓰고 있으니 실제 질문은 두 개입니다.

1. **L2a를 얼마나 강하게 쓸 것인가** — `q_default`를 태스크에 맞게 잡았는가, 행동 공간이 관절 위치인가 토크인가, IK를 끼울 것인가
2. **L2b를 추가할 것인가** — 태스크 수준 컨트롤러(임피던스·MPC)를 기저로 두고 RL에 잔차만 맡길 것인가

**1번이 비용 0에 가깝고 효과가 큽니다.** cable 태스크에서 그리퍼를 케이블 근처 자세로 `q_default`를 잡는 것만으로도
앞 문서 2절의 "보상 신호가 발생하는 상태에 데려다 놓기"가 부분적으로 달성됩니다 `[미확인]`.
...
**로코모션에서 `상용` 등급을 준 근거** `[문헌]` — 제 정의의 ②(라이브러리 기본값)가 실제로 충족됩니다.
Isaac Lab이 증류를 **설정 클래스와 진입점으로 제공**합니다.

| 제공물 | 용도 |
| --- | --- |
| `RslRlDistillationRunnerCfg` | 증류 알고리즘용 러너 설정 |
| `RslRlDistillationStudentTeacherCfg` | student/teacher 망 설정 (rsl-rl ≥ 4.0.0에서 deprecated → `RslRlMLPModelCfg`) |
| `rsl_rl_distillation_cfg_entry_point` | **교사를 RL로 학습 후 학생으로 증류하는 예제 스크립트** |

**즉 L4는 "논문에서 하는 것"이 아니라 우리 체크아웃에서 설정만 쓰면 되는 것입니다** `[미확인]` — 3.0-beta2에
이 클래스들이 실제로 있는지는 확인 대상입니다.

| 도메인 | 구현 | 특징 |
| --- | --- | --- |
| 로코모션 | 특권 상태 교사 → 노이즈 관측 학생 | 2022년 이후 주류. **위 표대로 라이브러리 기본값** |
...
| 7 | 3절 L2 절 전체 | **L2를 L2a(상용, 이미 쓰고 있음) / L2b(정착)로 분리.** "L2를 도입할까"가 아니라 "L2a를 얼마나 강하게 쓰고 L2b를 더할까"가 실제 질문 (이 문서 4.3절) |
...
| 8 | **매트릭스의 나머지 칸에 L2와 같은 정의 오류가 더 있는지.** 초판은 칸마다 판정 근거를 명시하지 않고 판단으로 채웠습니다 — **칸별로 "무슨 증거로 이 등급인가"를 적어야 감사 가능합니다** |
| 9 | `Isaac-Lift-Cloth-Franka-v0`의 action term이 관절 위치인지 effort인지 (4.1절) |
| 10 | Isaac Lab 3.0-beta2에 `RslRlDistillationRunnerCfg` 계열이 실제로 있는지 (6.1절) |
