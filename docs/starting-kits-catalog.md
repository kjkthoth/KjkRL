...
| --- | --- | --- |
| **L0 물리·시뮬레이터** | 강체·접촉·변형체 솔버, GPU 병렬 | 물리 엔진 |
| **L1 환경 프레임워크** | 관측·보상·종료·커리큘럼 조립 규격 | 환경 API 전체 |
| **L2 RL 라이브러리** | 학습 루프, PPO/SAC/증류 구현 | 알고리즘 구현·분산 학습 |
| **L3 로봇 스타터 킷** | 내 로봇의 URDF·액추에이터·기본 태스크 | 로봇 모델링·튜닝 |
| **L4 태스크·벤치마크** | 태스크 정의와 **평가 지표** | 성공 기준 자체 |
| **L5 데이터** | 시연 궤적·데이터 포맷 | 텔레옵 리그 + 수집 |
| **L6 사전학습 가중치** | 이미 학습된 정책 | 사전학습 전체 |
| **L7 데이터 생성 도구** | 시연 몇 개 → 합성 궤적 다량 | 대량 수집 |
| **L8 평가·운영 도구** | 통계적 비교, 실험 추적 | 평가 방법론 |
...
| 이름 | 내용 |
| --- | --- |
| **WireCraft** | **산업용 DLO(변형 선형 물체) 벤치마크.** 커넥터 삽입 / 클립 라우팅 / 채널 안착 3태스크군, 난이도·자산 조절, **관절형 + 변형체 두 물리 모델**, 시뮬 + 실물 UR5 궤적, **RL·IL·VLA를 공통 지표로 비교**. 특권 상태 RL 기준선 **82%+** |
| LIBERO · ManiSkill · RoboTwin · CALVIN | `rlinf`가 지원하는 조작 시뮬 스위트 |
| MuJoCo Playground 태스크 | 4족·휴머노이드·손·팔 |

> **⚠ WireCraft는 Isaac Lab 2.2.1 / Isaac Sim 4.5 기반입니다.** `[문헌]`
> `[실측]` 우리는 3.0.0-beta2 + Newton이라 **코드를 그대로 쓸 수 없습니다.**
> **가져올 것은 태스크 분류와 평가 지표입니다** — 프레임워크 버전과 무관하게 성립합니다.
...
| **GR00T N1.7** | 인간 1인칭 영상 20,000시간 + Isaac 합성 데이터 |
| **SmolVLA** | **약 4.5억 파라미터 경량.** LeRobot **커뮤니티 데이터로 학습**, 언어 조건 실물 제어 |
| XVLA · EO-1 · MolmoAct2 · WALL-OSS · EVO1 | |

**"ACT + 시연 50개"가 이 문서에서 가장 실무적인 숫자입니다** — L3(시연 기반)의 진입 비용이 그 정도입니다.
...
| 계층 | 상태 | 내용 |
| --- | --- | --- |
| L0 물리 | **받음** | Newton 1.2.1 — VBD 변형체가 선택 이유 |
| L1 환경 | **받음** | Isaac Lab 3.0.0-beta2 매니저 API |
| L2 RL 라이브러리 | **받음** | `rsl_rl` (+`rlinf`는 `contrib` extra 미설치) |
| L3 로봇 스타터 | **절반** | cloth 태스크 cfg는 있음 / **cable은 없음 — 직접 만들어야 함** |
| L4 벤치마크 | **절반** | WireCraft 존재하나 버전 불일치 → **지표만 차용** |
| L5 데이터 | **없음** | 변형체 시연 없음. `[미확인]` 공개 데이터에 있는지 확인 필요 |
| L6 사전학습 가중치 | **없음** | 승인으로 **가능해졌으나 아직 안 씀** |
| L7 데이터 생성 | **열림** | 요청 1 승인으로 Mimic·SkillGen 사용 가능. **아직 안 씀** |
| L8 평가 도구 | **없음** | rliable 미도입. 판정 기준 미정 |
...
4. **L5·L6·L7은 태스크 모드가 정해진 뒤에** — 상태 기반이면 필요 없고, 카메라·언어 기반이면 전부 필요
...
| # | 확인할 것 |
| --- | --- |
| 1 | **L0 상충** — 접촉 많은 조작은 MuJoCo 물리가 더 정확하다는 평가 vs 우리는 변형체 때문에 Newton. 실제 벤치로 확인 |
...
**벤치마크**
- [WireCraft (arXiv 2606.18097)](https://arxiv.org/abs/2606.18097) — **우리 태스크에 가장 가까운 기성 벤치마크**
